...
RSpec.describe User, type: :model do
  ...
  it "名がない場合は無効である" do
    ...
    deliver_address.valid?
    expect(deliver_address.errors[:last_name]).to include("can't be blank")
  end
  ...
  it "姓(カナ)がない場合は無効である" do
    ...
    deliver_address.valid?
    expect(deliver_address.errors[:first_name_reading]).to include("can't be blank")
  end
  ...
  it "名(カナ)がない場合は無効である" do
    ...
    deliver_address.valid?
    expect(deliver_address.errors[:last_name_reading]).to include("can't be blank")
  end
  ...
  it "郵便番号がない場合は無効である" do
    ...
    deliver_address.valid?
    expect(deliver_address.errors[:post_code]).to include("can't be blank")
  end
  ...
  it "都道府県がない場合は無効である" do
    ...
    deliver_address.valid?
    expect(deliver_address.errors[:prefecture_id]).to include("can't be blank")
  end
  it "市町村がない場合は無効である" do
    deliver_address = FactoryBot.build(:deliver_address, address_city: nil)
    deliver_address.valid?
    expect(deliver_address.errors[:address_city]).to include("can't be blank")
  end
  it "番地がない場合は無効である" do
    deliver_address = FactoryBot.build(:deliver_address, address_street: nil)
    deliver_address.valid?
    expect(deliver_address.errors[:address_street]).to include("can't be blank")
  end
end